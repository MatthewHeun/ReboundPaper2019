...
$\alpha \in (0,1)$ is an weighting parameter 
...
contexts. 
However, the Cobb-Douglas utility model is not usable for rebound analysis, because 
it leads to a direct rebound, $Re_{dsub} + Re_{isub} = 1$, by definition.
**** GS: you said 

direct rebound, $Re_{dsub} + Re_{isub} = 1$, by definition.

but $Re_{dsub} + Re_{isub}$ is substitution rebound, not direct rebound. 
Maybe change to ``substitution rebound ($Re_{sub} = Re_{dsub} + Re_{isub}$) equal to 1.''?
---MKH ****

To see that substitution rebound is 100\%,
note first that the uncompensated own-price elasticity ($\eqspsUC$)
in the Cobb-Douglas utility model is a constant $-1$.
...
\end{equation}
%
where $\rate{M}$ is the income rate as before. 
...
rebounds with simple elasticities and in particular
...
**** GS: But why are you using the approximate model, Eq.~\ref{eq:approx_qshat_over_qsorig}? 
We argue elsewhere that the approximate model is in appropriate.
How can we rely on the approximate model to make our point here? 
---MKH
****
