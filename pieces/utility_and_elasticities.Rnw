...
A second utility model discussed in this appendix is the
...
A third utility model (with Cobb-Douglas form) is
mentioned briefly for completeness.
We discard the Cobb-Douglas utility model, because
it always gives 100 \% rebound.
