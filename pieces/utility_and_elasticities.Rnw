...
$\alpha \in (0,1)$ is an output elasticity 
...
the compensated own price elasticity of energy service consumption ($\eqspsC$) and
the compensated cross-price elasticity of energy service consumption ($\eqopsC$).
