...
changes in service price are infinitesimally
small ($\Delta \aempl{\eta} \approx 0$ and $\Delta \aempl{p}_s \approx 0$).
In fact, infinitesimally small efficiency gains are
likely to be uneconomic: 
the increased capital expense will overwhelm the financial savings.
Accordingly, 
...
The approximate utility model is rarely applicable to real-world EEUs.
The approximate model is discussed for continuity with the literature only.
(See, for example, \citet{Borenstein:2015aa}.)
The other utility model is the CES utility model employed in the rebound framework.
(See Section~\ref{sec:sub_effect_main_paper}.)

Both utility models in the substitution effect
rely upon elasticities to estimate consumption utility
and the comparative benefits of 
...
elasticities describe the proportions of net income spent on the energy service
