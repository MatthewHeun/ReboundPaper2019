...
In this appendix, we describe three utility models.
...
Thus, a mathematically rigorous rebound analysis framework needs to accommodate 
...
with elasticity of substitution ($\sigma$) equal to 1. It simplifies
many derivations due to a number of strong assumptions on behavior.
The simplicity of Cobb-Douglas utility model has side effects:
among them is that it always gives 100 \% direct substitution rebound.
That characteristic disqualifies its use in a mathematical framework
...
+++ Not sure what you're trying to say with the now beginning paragraph, Matt.
In its current form I'm tempted to ask to delete it, but I must be
missing something. ---GS+++
...
($\frac{\rasub{q}_o}{\rbsub{q}_o} < 1$)
caused by the increase of energy service consumption
($\frac{\rasub{q}_s}{\rbsub{q}_s} > 1$)
...
Before proceeding with the utility models,
...
we solve for $\rate{C}_o$ and judiciously multiplying by $\rbempl{C}_o/\rbempl{C}_o$
...
The Cobb-Douglas model is a special case of the general CES utility model,
...
To see this, note that the Cobb-Douglas Marshallian demand of any good (illustrated
for the energy service here) is of the form
\begin{equation} \label{eq:CD_marshall}
  \rate{q}_s^m = \alpha M /p_s
\end{equation}
where M is the income and the superscript m denotes Marshallian demand.
The price elasticity of demand is therefore
\begin{equation} \label{eq:CD_marshall}
  \eqspsUC = \dfrac{\partial \rate{q}_s^m}{\partial p_s} \dfrac{p_s}{\rate{q}_s^m} = -\dfrac{\alpha M}{p_s^2} \dfrac{ p_s^2}{\alpha M} =-1
\end{equation}
...
+++ Matt, please check my notation on the CD elasticitiy. ---GS+++

****
Gregor to check Saunders (2008), which PB thinks may have said something similar. ---MKH 
****
...
In this paper, we describe two utiliity models in detail: 
an approximate model and a CES model.
All utility models require elasticities to describe consumer preferences.
In fact, there are three elasticies across the two utility models we discuss in detail:
...
Since the \emph{un}compensated price elasticity of energy service 
demand ($\eqspsUC$), tends to be more widely estimated,
e.g. in studies about driver behavior \citep{Gillingham:2020aa}, the usual
approach is to use it and the Slutsky equation to derive the compensated
elasticity. The CES model then imposes enough restrictions to determine
...
estimate the respending of net savings:
