...
\begin{equation} \label{eq:CD_marshall}
  \rate{q}_{s,m} = \alpha \rate{M} /p_s
\end{equation}
%
where M is the income and the subscript m denotes Marshallian demand.
The price elasticity of demand is therefore
\begin{equation} \label{eq:CD_marshall}
  \eqspsUC = \dfrac{\partial \rate{q}_{s,m}}{\partial p_s} \dfrac{p_s}{\rate{q}_s^m} 
    = -\dfrac{\alpha M}{p_s^2} \dfrac{ p_s^2}{\alpha M} =-1
\end{equation}
