...
One utility model is an approximate  model
that applies only when
changes in energy efficiency and 
changes in service price are marginal and small
such that $\Delta \aempl{\eta} \approx 0$ and $\Delta \aempl{p}_s \approx 0$.
We note that small efficiency gains are
unlikely to be economic: 
the capital cost of the EEU will overwhelm expected financial savings.
Accordingly, 
the real-world EEUs in this paper have very large percentage increases in energy efficiency.
For the car in Section~\ref{sec:car_example}, 
energy efficiency increases from 
$\orig{\eta} = \Sexpr{mpg_worse}$~mpg to
$\aprod{\eta} = \Sexpr{mpg_better}$~mpg, 
...
Thus, the approximate utility model is rarely applicable to real-world EEUs.
The approximate model is discussed for continuity with the literature only.
(See, for example, \citet{Borenstein:2015aa}.)

The other utility model is a 
Constant Elasticity of Substitution (CES)
utility model that underlies the substitution effect
in this rebound framework.
The CES utility model applies when changes in the energy service price
and energy efficiency are non-marginal,
as all real-world EEUs are expected to be.
(See Section~\ref{sec:sub_effect_main_paper}.)

In the substitution effect, Both utility models
...
In this appendix,
the approximate and CES utility models are described.
...
(See Appendix~\ref{sec:non_energy_service_expenditure} for details.)
...
We note briefly (because we don't use it in this paper), 
...
In this paper, we describe two utiliity models, 
an approximate model and CES model.
Both models require elasticities to describe consumer preferences.
