...
(See, for example, ******** Gregor: can you point to a paper that uses the elasticity equation? ****.)
...
The Cobb-Douglas utility model has an unfortunate feature:
it always gives 100 \% direct substitution rebound.
...
**** Revisit after Gregor edits text below. ****
...
elasticities describe the proportions of net income ($\rasub{N}$)
...
\begin{equation} 
  \frac{\rasub{q}_s}{\rbempl{q}_s}
      = \left( \frac{\amacro{\eta}}{\bempl{\eta}} \right)^{-\eqsps} 
...
\begin{equation}
  \frac{\rasub{C}_o}{\rorig{C}_o}
      ...
      = \left( \frac{\amacro{\eta}}{\bempl{\eta}} \right)^{-\eqops} \tag{\ref{eq:Cdot_o_ratio}}
\end{equation}
...
\iicirc{}
at point $\circ$
in Figs.~\ref{fig:ExamplePrefsPathGraph}, 
...
(See Figs.~\ref{fig:ExamplePrefsPathGraph}, 
\ref{fig:CarPrefsGraph}, and 
\ref{fig:LampPrefsGraph}.)
In $\rate{C}_o/\rbempl{C}_o$ vs.\ $\rate{q}_s/\rbempl{q}_s$ space, 
...
****
Gregor: We need to substantiate the claim that $Re_{dsub} = 1$ always
for the CD utility model. 
Is there a small proof that we can include?  
Or is it widely known?
If it is widely known to economists
(but maybe not to energy people), 
perhaps we should include the proof
for completeness in this appendix. 
****
...
%++++++++++++++++++++++++++++++
\subsection{Elasticities for the substitution effect ($\eqsps$, $\eqops$, and $\sigma$)}
...
None of the elasticities required for the approximate and CES utility models
...
**** Gregor to provide a reference to Slutsky eqaution ****, 
