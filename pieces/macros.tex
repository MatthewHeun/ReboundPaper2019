...
% \newcommand{\md}{O\!M\!d}
\newcommand{\md}{md}
% Can't be \prod, because \prod is already a command for the product symbol.
...
\newcommand{\Redinceqnexact}{\frac{ \left( 1 + \frac{\rbinc{N}}{\Mdothatprime} \right) ^{\eqsM} - 1}
              { \etaratiostacked - 1 } \left( \frac{\rasub{q}_s}{\rorig{q}_s} \right) }
...
\newcommand{\Cosolution}{\left\{ \left[ 1 + \fCs \left\{ \left[ \left( \frac{1-\fCs}{\fCs} \right)
          \frac{\aprod{p}_s \rbempl{q}_s}{\rbempl{C}_o} \right] ^{1-\sigma} - 1 \right\} \right]^{-1}  \right\} 
                                                ^{1/\rho}}
...
\newcommand{\iibar}{\seg{$\bprod{\text{i}}$}{black}{0.3 mm}{$\,\bprod{\text{i}}$}}
...
\newcommand{\bartilde}{\seg{$- \,$}{prodcolor}{0.6 mm}{$\, \sim$}}
