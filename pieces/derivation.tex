...
% \input{pieces/deriv1_device_effect.tex}       % Delete later.


% \input{pieces/deriv2_lifecycle_effect.tex}    % Delete later.
...
when energy service efficiency increases ($\aprod{\eta} > \bempl{\eta}$),
...
\begin{equation} \label{eq:Cdot_o_ratio}
  \frac{\rasub{C}_o}{\rbsub{C}_o} 
      = \frac{\rasub{q}_o}{\rbsub{q}_o} 
      = \left( \frac{\aprod{\eta}}{\bempl{\eta}} \right)^{\epsilon_{p_s} (\alpha / \beta)}  \; .
\end{equation}
%
...
%
\begin{equation}
  \rasub{C}_o = \rbempl{C}_o \left( \frac{\aprod{\eta}}{\bempl{\eta}} \right)^{\epsilon_{p_s} (\alpha / \beta)} \; .
\end{equation}

Substituting Eq.~\ref{eq:Cdot_o_ratio} into Eq.~(\ref{eq:Re_isub_prelim}) gives
%
\begin{equation}
  Re_{isub} = \frac{\left[ \left( \frac{\aprod{\eta}}{\bempl{\eta}} \right)^{\epsilon_{p_s} (\alpha / \beta)} - 1  \right] \rbsub{C}_o I_E} {\Sdot} \; .
\end{equation}
% %
...
%
\begin{equation}
  Re_{isub} = \frac{x^m - x}{x - 1} \frac{\rbempl{C}_o I_E}{\rbempl{E}_s} \; ,
\end{equation}
%
where $m \equiv \epsilon_{p_s} \alpha/\beta + 1$.
Taking derivatives of numerator and denominator of Eq.~(\ref{eq:Re_isub}) gives
%
\begin{equation}
...
\end{equation}
%
% The result for $\epsilon_{p_s} \alpha/\beta + 1 > 1$ is included for completeness.
