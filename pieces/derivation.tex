...
The CES utility model assumes (incorrectly) that 
...
\end{equation}
%
Eliminating terms and recognizing that 
$\rorig{q}_s = \raempl{q}_s$ gives

\begin{equation} 
  Re_{dinc} = \Redinceqnexact{} \; . \tag{\ref{eq:Re_dinc}}
\end{equation}
%
Eq.~(\ref{eq:q_s_solution}) 
can be substituted into Eq.~(\ref{eq:Re_dinc})
...
\end{equation}
%
Sutstituting $\frac{\raempl{C}_o}{\rorig{C}_o} \rorig{C}_o$ for $\raempl{C}_o$ and 
...
\end{equation}
%
Eq.~(\ref{eq:C_o_solution}) can be substituted into \ref{eq:Re_iinc}
to complete the expression.
If there is no net income ($\rasub{N} = 0$), 
...
\begin{itemize}
  ...
  \item $Re_{dsub}$ (Eqs.~(\ref{eq:Re_dsub_prelim}) and (\ref{eq:q_s_solution})),
  
  \item $Re_{isub}$ (Eqs.~(\ref{eq:Re_isub_prelim}) and (\ref{eq:C_o_solution})), 
  
  \item $Re_{dinc}$ (Eqs.~(\ref{eq:Re_dinc}) and (\ref{eq:q_s_solution})), and
  
  \item $Re_{iinc}$ (Eqs.~(\ref{eq:Re_iinc}) and (\ref{eq:C_o_solution})),
