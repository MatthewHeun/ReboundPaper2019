...
that arises because the energy conversion device owner spends net
income from the EEU.
The income of the device owner is $\rbempl{M}$, 
...
\end{enumerate*}
%
But we first derive several helpful expressions for later use.
...
\end{equation}
%
Canceling terms and substituting Eqs.~(\ref{eq:Sdot}) and~(\ref{eq:qsrat_eqsM}) gives
...
\begin{equation}
  Re_{iinc} = \frac{\left[ \left( 1 + \frac{\rbinc{N}}{\Mdothatprime} \right)^{\eqoM} - 1  \right] 
                    \left( \etaratiostacked \right)^{-\eqops - 1}}{\etaratiostacked - 1} 
