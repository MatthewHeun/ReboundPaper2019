...
are cancelled to 0, e.g., $\cancelto{0}{\Delta \raempl{C}_o}$. 
Most cancellations can be traced back to Table~\ref{tab:analysis_assumptions}.
Table~\ref{tab:cancellations} highlights those assumptions.

\begin{table}
\centering % Centered table
\caption{Sources for cancellations in Tables~\ref{tab:empleffect}--\ref{tab:prodeffect}.}
\begin{tabular}{r l}
  \toprule
  Cancellation & Source or justification \\
  \midrule
  $\cancelto{0}{\Delta \raempl{C}_o}$ & Table~\ref{tab:analysis_assumptions}: 
                                $\rorig{C}_o = \raempl{C}_o$ ($\rate{C}_o$ unchanged across emplacement effect.) \\
  $\cancelto{0}{\rorig{N}}$ & Table~\ref{tab:analysis_assumptions}: 
                                $0 = \rorig{N}$ (Net income is zero prior to the EEU.) \\
  $\cancelto{0}{\Delta \rasub{E}_{emb}}$ & Table~\ref{tab:analysis_assumptions}: 
                                $\rbsub{E}_{emb} = \rasub{E}_{emb}$ ($\rate{E}_{emb}$ unchanged across substitution effect.) \\
  \bottomrule
\end{tabular}
...
Thereafter, we note an assumption of exogenous pries of other goods ($p_o$)
and develop an approximate model and a CES utility model for determining the 
...
\begin{equation} \label{eq:Re_dsub_def}
  Re_{dsub} = \frac{\Delta \rasub{E}_s}{\Sdot} = \frac{\rasub{E}_s - \rbsub{E}_s}{\Sdot} \; .
\end{equation}
%
...
\begin{equation} \label{eq:Re_isub_def}
  Re_{isub} = \frac{\Delta \rasub{C}_o I_E}{\Sdot} = \frac{(\rasub{C}_o - \rbsub{C}_{o}) I_E}{\Sdot} \; .
\end{equation}
%
Rearranging gives
...
\end{equation}
%
Substituting Eq.~($\ref{eq:Sdot}$) and rearranging gives
...
%..............................
\paragraph{Exogenous $p_o$ assumption}
...
such that $\asub{p}_o = \orig{p}_o$,
the preceding two equations can be solved for prices ($\orig{p}_o$ and $\asub{p}_o$)
and set equal to find
...
%..............................
\paragraph{Exact (CES) utility model}
