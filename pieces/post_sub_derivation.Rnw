...
Four equations are helpful for this analysis.
First, the slope at any point of indifference curve \iicirc{} 
...
\begin{align} \label{eq:slope_indifference_curve_u1}
  \frac{\partial (\rate{C}_o/\rbempl{C}_o)}{\partial (\rate{q}_s/\rbempl{q}_s)} =&
        ...
        &\times \left[ \left( \frac{1}{1 - a} \right) 
                - \left( \frac{a}{1 - a} \right) 
                          \left( \frac{\rate{q}}{\rbempl{q}_s} \right)^\rho \right]^{(1 - \rho)/\rho} \; .
\end{align}

Second, the equation of \circcirc{} expenditure line is

\begin{equation} \label{eq:origorig_line}
  \frac{\rate{C}_o}{\rbempl{C}_o} = 
      -\frac{\bempl{p}_s \rbempl{q}_s}
            {\rbempl{C}_o}
        \left(  \frac{\rate{q}_s}{\rbempl{q}_s} \right)
      + \frac{1}{\rbempl{C}_o} 
        (\rate{M} - \rbempl{C}_{cap} - \rbempl{C}_{md} - \rbempl{C}_{md}) \; .
\end{equation}

Third, the equation of the \starstar{} expenditure line is
...
\end{equation}
%
Note that both the \circcirc{} line and the \starstar{} line
pass through the $\circ$ and $*$ points in
Figs.~\ref{fig:ExamplePrefsPathGraph},
\ref{fig:CarPrefsGraph}, and
\ref{fig:LampPrefsGraph}.

Fourth, the equation of the \hathat{} expenditure line is given by 

\begin{equation} \label{eq:hathat_line}
  \frac{\rate{C}_o}{\rbempl{C}_o} = 
      -\frac{\aprod{p}_s \rbempl{q}_s}
            {\rbempl{C}_o}
        \left(  \frac{\rate{q}_s}{\rbempl{q}_s} \right)
      + \frac{1}{\rbempl{C}_o} 
        (\rate{M} - \rbempl{C}_{cap} - \rbempl{C}_{md} - \rate{G} 
          + \aprod{p}_s \Delta \rasub{q}_s + \Delta \rasub{C}_o) \; .
\end{equation}
%
Note that line \hathat{} (Eq.~\ref{eq:hathat_line})
has the same slope as line \starstar{} (Eq.~\ref{eq:starstar_line}), 
but a different $y$-intercept.

To find the rate of energy service consumption after the substitution effect
($\rasub{q}_s$), set the slope of the \hathat{} line (Eq.~\ref{eq:hathat_line})
equal to the slope of the \iicirc{} indifference curve
when $\rate{u}/\rbempl{u} = 1$ (Eq.~\ref{eq:slope_indifference_curve_u1}).

\begin{equation}
  -\frac{\aprod{p}_s \rbempl{q}_s}
        {\rbempl{C}_o} = 
    -\frac{a}{1 - a} \left( \frac{\rate{q}_s}{\rbempl{q}_s} \right)^{(\rho -1)} % \nonumber  \\
        \left[ \left( \frac{1}{1 - a} \right) 
                - \left( \frac{a}{1 - a} \right) 
                          \left( \frac{\rate{q}}{\rbempl{q}_s} \right)^\rho \right]^{(1 - \rho)/\rho}
\end{equation}
%
...
\begin{equation} \label{eq:q_s_solution}
  \frac{\rasub{q}_s}{\rbempl{q}_s} = \left\{ a + (1-a) 
      \left[ \left(  \frac{1-a}{a}  \right) \frac{\aprod{p}_s \rbempl{q}_s}{\rbempl{C}_o}  \right]
                                                  ^{\frac{\rho}{1-\rho}} \right\} ^ {-1/\rho} \; .
\end{equation}

The rate of other goods consumption after the substitution effect ($\rasub{C}_o$) 
can be found by substituting $\rate{u}/\rbempl{u} = 1$ 
and Eq.~(\ref{eq:q_s_solution})
into the functional form of the utility model (Eq.~(\ref{eq:utility_Co_form}))
to obtain

\begin{equation}
  \frac{\rasub{C}_o}{\rbempl{C}_o} = \left\{ \left( \frac{1}{1-a} \right) 
                                     - \left( \frac{a}{1-a} \right)   
              \left\{ a + (1-a)
                  \left[ \left( \frac{1-a}{a} \right) \frac{\aprod{p}_s \rbempl{q}_s}{\rbempl{C}_o}   \right] 
...
\begin{equation} \label{eq:C_o_solution}
  \frac{\rasub{C}_o}{\rbempl{C}_o} = \left\{ \frac{1}{1 + a \left\{ \left[ \left( \frac{1-a}{a} \right)
