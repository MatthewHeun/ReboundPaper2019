...
Direct substitution effect rebound is given by
...
developing approximate and exact methods of determining
direct substitution rebound.

Indirect substitution effect rebound is given by
...
share parameter $a$ replaced by $\fCs$,
...
the energy service price elasticity of other goods consumption ($\eqops$)
are constant along an indifference curve.
This assumption is an approximation that holds only 
for infinitesimally small energy service price changes 
($\Delta \aempl{p}_s \approx 0$).
However, in the case of an energy efficiency upgrade (EEU), 
the energy service price change is not infinitesimal.
...
\end{equation}
%
into which Eq.~(\ref{eq:q_s_solution}) can be substituted directly.
...
\end{equation}
% 
into which Eq.~(\ref{eq:C_o_solution}) can be substituted directly.
