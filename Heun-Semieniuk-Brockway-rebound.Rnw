...
\Bt{} likens this to a ``multiplier'' effect,
whereby the leftover purchasing power
...
In fact, energy efficiency is a technological improvement, and 
empirical evidence supports the conceptual argument that technological improvements 
that raise efficiency and reduce service price
...
\footnote{
  ...
  for why output should stay constant and therefore one important rationalisation of Basu et al. of their results.
}
...
moreover it can be very small relative to the macroeconomy. 
For instance, if everyone in the US saved \$10 
...
Under those circumstances, a multiplier effect might be a reasonable approximation 
