...
Energy rebound for each effect $j$ (embodied energy, substitution, technology)
...
(embodied energy, substitution, income, and productivity).
...
\end{equation}
%
Then, total rebound ($Re$) is given by 
%
...
the sign of $Re_j$ is determined by the sign of the numerator, takeback$_j$.
...
%++++++++++++++++++++++++++++++
\subsection{Device-level effect}
\label{sec:device_level_effect}
%++++++++++++++++++++++++++++++
...
As discussed in Section~\ref{sec:device_level_effect} above,
$U$ reduces the service price ($p_s$)
leading to increased consumption of energy services provided by the device
($\rateafter{q}_s > \ratebefore{q}_s$).
However, increased energy service consumption 
implies increased direct final energy cost for the device.
...
Our approach to substitution effects is identical to \Bt{}.
...
The net substition effect is given by
...
\footnote{
  ...
  \citet{Gillingham2016a} and \citet{Gillingham2016b}, who cite \Bt{}.
