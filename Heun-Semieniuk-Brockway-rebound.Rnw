...
We consider five rebound effects:
%
\begin{enumerate*}[label={(\arabic*)}]

  \item embodied energy effects,

  \item device-level effects,

  \item substitution effects, 
  
  \item income effects, and 
...
who considered only three effects in his derivation and calculations:
...
Energy rebound for each rebound effect $j$ (embodied energy, substitution, technology)
...
(embodied energy, substitution, and technology).
...
The final energy consumption rate of the device is
...
**** Pull discussions from later sections into derivation sections. ****

**** Say that embodied energy rebound could easily be negative, because
$\Delta \rate{E}_{\femb}$ could be negative

**** Consider ``technology rebound'' and ``technology factor'' 
instead of ``productivity,'' following van Benthem. ****

**** GS says following is difficult from substitution effect onward. 
Consider adding more steps? ****

**** Collapse device-level and substitution effect into one section ****
...
such that $\after{E}_{\femb} > \before{E}_{\femb}$.
...
Equation~\ref{eq:Re_emb} shows that both
more embodied energy ($\after{E}_{\femb} > \before{E}_{\femb}$)
and
shorter device lifetime ($\after{t} < \before{t}$)
increase embodied energy rebound.
But embodied energy rebound can be either positive or negative.
When the embodied energy rate for a device
goes up (down) due to $U$,
total rebound also increases (decreases)
by the same amount
(but not usually by the same percentage).
...
While the effects of embodied energy on embodied energy rebound
...
on substitution rebound ($Re_{sub}$) 
and, therefore, on overall rebound ($Re$),
is less clear.
...
Income ($\rate{G}$) is money that is no longer spent on energy to operate the upgraded device,
...
Our approach to capital and disposal costs is simlar to our approach to embodied energy, i.e.\
...
the change in device cost rate, or net income.
...
Higher energy prices ($p_{\Ef}$) yield greater income effect, because
energy cost savings yield more freed cash.
Similarly, higher energy intensity ($I_{\Ef}$) gives larger income effect, because
each dollar of energy cost savings spent in the economy has higher energy implications.

$\Delta \rate{E}_{\finc}$ represents energy savings taken back due to the income effect.
...
income effect rebound can be expressed as
%
\begin{equation} \label{eq:Re_i}
  Re_i = \Reiequation{} \, .
\end{equation}
%
...
($\rate{C}_{dev}$, represented by $\gamma$ in Equations~\ref{eq:Re_i}--\ref{eq:Re_tot})
go up (down),
...
which boosts $Re_{i}$ in Equation~\ref{eq:Re_i}
...
$1 - Re_{dev}/k - \gamma$ will also be positive. 
...
%++++++++++++++++++++++++++++++
\subsection{Productivity effect}
\label{sec:productivity_effect}
%++++++++++++++++++++++++++++++

**** 
Note and question from Matt to Gregor:
The passage from \Bt{p.~11} is as follows:
``The magnitude of a macroeconomic multiplier for government expenditures is quite controversial, 
though recent studies seem to suggest that it is probably not much above 1, 
except in times of very high unemployment. 
The multiplier in this case is somewhat different, however, 
because it does not rely on expanding government expenditures and (generally associated) government debt. 
This would be more akin to the macroeconomic multiplier that might result following a new discovery of a natural resource deposit.''
So it seems that Borenstein \emph{also} realizes that this multiplier is not exactly like a Keynsian multiplier.
Seems like we are in basic agreement with \Ba{} on this point, but 
the text below is somewhat adversarial.
Do you want to amend the discussion below to more fairly represent \Ba{}'s views?
****

Another macroeconomic rebound effect according to \Bt{}
is a ``multiplier'' effect, 
whereby the leftover purchasing power that the energy consumer has, 
which is typically analyzed only as an indirect rebound effect, 
acts like a spending multiplier from Keynesian theory. 
The idea runs as follows: 
with less than full utilization of resources, 
the consumer spends part of the leftover money on additional goods, 
i.e.\ raises effective demand, 
puts additional resources to work, and, hence,
raises output which requires additional energy inputs. 
Several of these spending rounds occur,
raising the rebound to a higher level than would have been the case
had we only considered the effects of the consumer's re-spending. 
...
This idea has also been promulgated by \citet{Gillingham:2015aa,Gillingham2016a,Lange2019}, 
who cite \Ba{}.%
\footnote{
  Interestingly, \citet{VanBenthem2015} cites \Ba{} and discusses the macroeconomic rebound effect 
  but does not refer to the multiplier.
}

However, the invocation of a multiplier effect being caused directly 
by the money savings of the consumer 
is fallacious in this context. 
The Keynesian multiplier **** Borenstein says specifically that energy efficiency 
is \emph{different from} a Keynsian multiplier. ---MKH ****
operates whenever \emph{additional} purchasing power
...
These additional funds can mobilize additional resources. 

In contrast, an energy efficiency improvement on the contrary does \emph{not} add new purchasing power, 
at least in the short-run, where the Keynesian multiplier operates.
The consumer just redirects existing money and spends it on a different basket of goods.
The contention that consumer purchases of additional goods in the efficient case
...
receive less income and must curtail their spending. 
In a closed economy,
...
It is a supply side, not demand side shock.%
\footnote{
...
Empirical evidence supports the conceptual argument that 
technological improvements that raise efficiency, 
thereby reducing the price of energy services,
...
In other words, the ``multiplier effect'' understood in the Keynesian sense, 
is a misnomer when applied to energy efficiency, 
because the ``multiplier effect''
refers to changes in expenditure and real income by changing the monetary \emph{income}, 
whereas energy efficiency changes real income by reducing monetary \emph{expenses},
in the sense that the energy service price is reduced.%
\footnote{
...
None of the above denies the importance of a macroeconomic effect. 
Indeed, the macroeconomic effects of energy efficiency are productivity effects,
just like other factor-augmenting productivity improvements. 
Energy efficiency operates along the supply side by reducing the cost of inputs (specifically, energy)
and thus allowing production on a greater scale. 
This ***** what is antecedent for ``this?''? ****  can lead 
to important counter tendencies in factor use reduction.
...
Of course, this generality to labor raises the question 
whether it is appropriate to speak of a rebound effect at all. 
Is all technological progress just a rebound effect? 
We put it the opposite way: macroeconomic rebound effects
are the consequences of technological change, 
whether in utilization of labor or utilization of energy.
**** Can we also include a capital example here? 
...
****
MKH note to Gregor:
We agreed to try splitting the productivity effect from the income effect
in our narrative. 
Now, I'm questioning that decision.
I don't think it makes sense to have an income effect
and an \emph{additional} productivity effect for which $0 \le k < \infty$.
There is no ``income effect,'' per se!
Or, more precisely, there is no ``income effect'' independent of macroeconomic dynamics!
We claim that the device owner's spending of freed cash on other goods besides final energy
is canceled out by the device owner's no longer spending money on final energy.
Thus, the only rebound implications of the device-owner's freed cash is a macroeconomic effect.

Seen in this light, we can now say that \Ba{}'s income effect is not an income effect at all!
It is rather a productivity effect with assumed magnitude 1.

The argument I'm making here is a subtle one. 
But I think it is aligned with our developing understanding of 
the macroeconomic mechanisms at play here.

So I propose a discussion about these subtleties at this point of the text.

Do you agree?  

If we make this change, I need to make some changes to the text.
Our equation will become 

$Re = Re_{emb} + Re_{sub} + Re_p$

and the main equation will revert to its previous form (i.e., $k + 1$ terms will be replaced by $k$:

$Re = Re_{emb} + Re_{dev} + \left( 1 - \gamma - \frac{Re_{dev}}{k} \right) \kpI$.
...
Total rebound is given by $Re = Re_{emb} + Re_{sub} + Re_i$ + $Re_p$, or
%
\begin{equation}
  Re = \Reembequation{} + \Resubequation{} + \Reiequation{} + \Repequation{} \, .
\end{equation}
%
...
%------------------------------
\subsubsection{Effect of embodied energy} 
\label{sec:effect_of_embodied}
%------------------------------



%------------------------------
\subsubsection{Effect of device costs} 
\label{sec:effect_of_device_costs}
%------------------------------
...
%------------------------------
\subsubsection{Effect of device-level rebound} 
\label{sec:effect_of_device_level_rebound}
%------------------------------
...
%------------------------------
\subsubsection{Effect of elasticity} 
...
%------------------------------
\subsubsection{Effect of economic parameters} 
\label{sec:effect_of_economic_parameters}
%------------------------------
