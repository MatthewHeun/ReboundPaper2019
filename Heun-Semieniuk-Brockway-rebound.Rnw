...
This section shows the derivation of the \name{}.
...
The productivity effect is a macroeconomic rebound effect.

We use, as our basis, the model of \Bt{},
who considered only three effects: device-level, substitution, and income effects.
...
\footnote{
  Relaxing the exogenous price assumption
...
Energy rebound for each effect $j \in [1, 5]$ can be calculated by
...
like Equation~\ref{eq:rebound_1}.
...
After invoking the exogenous final energy price assumption
...
$\rate{E}_{\fgain}$ represents energy savings taken back by the device-level effects.
...
implies an increased direct final energy cost for the device.
The device owner's budget constraint means that  
increased direct final energy costs
will be shifted from other expenditures elsewhere in the economy.
...
After simplification, net substitution rebound can be expressed as
%
...
To the device owner, energy cost savings come at a cost to 
...
These device costs can be expressed as a rate ($\rate{C}_{dev}$), which is
typically, but not always, higher for the upgraded device
...
% After rearrangement, 
% $\Delta \rate{C}_{dev} = (\rateafter{C}_{dev} / \ratebefore{C}_{dev} - 1) \ratebefore{C}_{dev}$.

Freed cash ($FC$) is the difference between the income rate and
the change in device cost rate.
Thus, $FC = \rate{G} - \Delta \rate{C}_{dev}$.
As shown by \citet{Antal:2014aa}, 
it is reasonable to assume that 
freed cash is spent in the economy at the average energy intensity of the economy.
Thus, $\Delta \rate{E}_{\finc} = FC \, I_{\Ef}$.
...
Note that $\gamma < 1$ is expected, else $U$ will not be
expected to be money saving for the device owner.
