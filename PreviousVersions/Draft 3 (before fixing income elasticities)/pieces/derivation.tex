...
of energy service consumption ($\eqspsC$) and
other goods consumption ($\eqopsC$).
...
\begin{equation}
  \frac{\rasub{q}_s}{\rbsub{q}_s} = \left( \frac{\amacro{\eta}}{\bempl{\eta}} \right)^{-\eqspsC} \; .
...
energy service price elasticity of energy service consumption~($\eqspsC$) and the
...
is expected to be positive ($\eqopsC > 0$).
...
\begin{equation}
  \frac{\rasub{q}_o}{\rbsub{q}_o} = \left( \frac{\amacro{\eta}}{\bempl{\eta}} \right) ^ {-\eqopsC} \; . 
...
\begin{equation}
  \frac{\dbydetaeta{}\left[ \left( \etaratiostacked{} - 1 \right)^{-\eqspsC}  \right]}
           {\dbydetaeta{}\left[ \etaratiostacked{} - 1\right]}
    = -\eqspsC \left( \etaratiostacked{} \right)^{-(\eqspsC + 1)}
\end{equation}
%
...
\begin{equation}
  \lim_{\amacro{\eta}/\bempl{\eta} \to \infty} Re_{dsub} 
    = -\eqspsC ( \infty )^{-(\eqspsC + 1)}
    = 0 \; ,                                              \tag{\ref{eq:lim_Redsub_infty}}
\end{equation}
%
because $\eqspsC \in [-1, 0)$ and, therefore, $(\eqspsC + 1) > 0$.
...
\begin{equation}
  \lim_{\etaratioinline \to 1^{+}} Re_{isub} 
        = -\eqopsC \frac{\rbempl{C}_o I_E}{\rbempl{E}_s} \; . \tag{\ref{eq:lim_Reisub_1}}
\end{equation}
...
\end{equation}
%
Because $\eqopsC \in (0, 1)$ is expected **** Check with Gregor.  Is this true? ****,
...
\begin{equation} \label{eq:Re_d}
  Re_d = \frac{ \left( \etaratiostacked \right)^{-\eqspsC}
