...
\begin{tabular}{r l}
  ...
  $\sigma$ & elasticity of substitution between the energy service (s) and other goods (o) [--] \\
  \bottomrule
\end{tabular}
\label{tab:greek}
\end{table}
...
\begin{tabular}{r l}
  ...
  $\aprod{X}$ & $X$ after the \macroeffect{} \\
...
$\Delta \aprod{X} \equiv \aprod{X} - \bmacro{X}$.
...
\Delta X &= (\aprod{X} - \bmacro{X}) + (\ainc{X} - \binc{X})
            + (\asub{X} - \bsub{X}) + (\aempl{X} - \bempl{X}) \nonumber \\
\Delta X &= (\aprod{X} - \cancel{\bmacro{X}}) + (\cancel{\ainc{X}} - \cancel{\binc{X}})
