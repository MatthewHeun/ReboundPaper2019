...
Energy is embodied in the device during manufacturing, distribution, and disposal processes,
prior to consumer acquisition of the device and after the useful life of the device.
Aside from maintenance effects, 
no energy is embodied in the device while in service. 
However, for simplicity, we spread all embodied energy
over the lifetime of the device, 
an equal amount assigned to every period. 

We later take the same approach to device acquisition and disposal costs.
In reality, some devices (typically, light bulbs) are paid in full at the time of acquisition, 
but others (automobiles) are not and paid off in installments.%
\footnote{
  Since we are interested in energy consumption and not welfare effects, we abstract from discounting.
}
By spreading device acquisition and disposal costs across the life of the device, 
we treat all purchase plans equally.
Another justification for spreading purchase costs comes from considering
staggered device replacements by many consumers across several years.  
in the aggregate, staggered replacements 
work out to about the same embodied energy and cost in every period.
...
We now set about the task of developing the conceptual framework 
for linking microeconomic effects at the device level 
...
But we will soon note that the income effect does not operate in isolation
...
Next, we claim that energy-efficiency-driven productivity gains 
provide the most fruitful way to conceptualize macroeconomic rebound effects. 
Finally, we develop a mathematical expression for productivity rebound ($Re_p$).
...
also apply to income effects at the macro level. 
...
And we argue that in isolation the microeconomic ``income effect''
is not a good conceptual framework for understanding
the impact of energy efficiency in the broader economy.
...
The above is not to say that macroeconomic effects are unimportant.
Other authors have speculated about a ``multiplier'' effect, 
whereby the consumer's leftover purchasing power 
acts like a spending multiplier to enhance final energy rebound. 
...
puts idle resources to work, and, hence,
raises output,
which, in turn, requires additional final energy consumption. 
Several of these spending rounds occur,
each raising rebound to a level higher than would have been the case
if the consumer's initial spending were the only effect considered. 
...
Second, the Keynesian ``multiplier effect'' refers to changes in real income
caused by changing \emph{income}, 
whereas energy efficiency changes real income by reducing \emph{expenses},
because the energy service price is reduced.%
\footnote{
  **** Do we need this footnote? Seems like a distraction. Or like we are picking a fight with \citet{Basu2006}. ---MKH ****
...
**** Do we need this discussion of open economy? ---MKH ****
In an open economy, 
...
**** Do we need this discussion of savings here?
It comes out of the blue!  
If we keep it, we should probably signal better. ---MKH ****
...
Understood in this way, energy efficiency is a supply side shock, 
while the Keynesian multiplier is a demand side shock.%
\footnote{
...
we argue that a macroeconomic multiplier is not a helpful way
to conceptualize the link between device-level microeconomic effects and macroeconomic rebound.
\Ba{} seems to agree when he says, 
...
existing (unmodified) infrastructure (e.g., refineries) could process 
the newly-discovered primary energy resource.
The ``very low extraction costs'' phrase is also important, 
...
by reducing the price of inputs.

Reducing the price of inputs is a productivity enhancer, 
not a macroeconomic multiplier,
a point that we develop in the following section.
...
assuming the same level of service from the device.
...
the difference between the gross income rate and
the change in device cost rate.
...
we express this scaling simply with a multiplicative productivity factor ($k$), 
...
($\rate{C}_{dev}$, represented by $\gamma$ in Equation~\ref{eq:Re_p})
go up (down),
productivity rebound decreases (increases), because
...
rebound terms, as we do in Section~\ref{sec:extended_framework}.
...
the explicit rebound equations developed in Section~\ref{sec:extended_framework}.
Only the expressions for substitution and income effects
($Re_{sub}$ and $Re_{i}$) in Equations~\ref{eq:Re_sub} and~\ref{eq:Re_p}
...
%------------------------------
\subsubsection{Verification of productivity effect rebound ($Re_{p}$) expression}
\label{sec:verification_productivity}
%------------------------------

We derived an explicit equation for productivity effect rebound in Equation~\ref{eq:Re_p}.
The derivation is identical to the \Bt{} income effect with $k = 1$.
Thus, \Ba{}'s income effect rebound ($Re_i$) is given by
Equation~\ref{eq:Re_p} with $k = 1$, or 
$Re_i = Re_p(k = 1)$.
To verify the derivation of the explicit productivity effect rebound equation,
we use numbers from \Ba{}'s
fuel economy example to calculate $Re_{p}$ by Equation~\ref{eq:Re_p} assuming $k = 1$.
Then, we compare our answer to
\Ba{}'s numerical estimate for $Re_{i}$.
Agreement will indicate that the derivation of Equation~\ref{eq:Re_p} is consistent with \Ba{}'s 
model for income effect rebound.

Numerical quantification of each term in Equation~\ref{eq:Re_p} (except $\gamma$)
...
Using the above numbers in Equation~\ref{eq:Re_p} with $k = 1$,
we obtain $Re_{i} = 0.110$,
exactly matching the value of 0.11 reported in \Bt{p.~17, Table~2}.
The match verifies the derivation of Equation~\ref{eq:Re_p}.
