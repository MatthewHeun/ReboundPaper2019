...
While he and many others have examined the indirect reboud from income effects from a micro perspective, 
...
Here, as is wwell known from IS-LM macroeconomics,
...
reversed by crowding out \cite{Ramey2011,Ramy2019}, is that there is no
...
We conduct sensitivity studies conducted to gain understanding of the effect
...
so our model also illustrates how the demand side contributes to the wider ``macroeconomic
...
costless efficiency improvements and those costly ones mandated by policy. This is important
...
Thirdly, only very little data is needed to operationalise the model as we show with examples
