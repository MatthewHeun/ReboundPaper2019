...
Our work most closely follows the papers of \Tt{} and \Bt{}.
\Tt{} studied direct and indirect energy rebound effects
...
and he calculated numeric estimates for substitution and income rebound
...
He mentioned a ``macroeconomic multiplier'' \Bt{p.~11}
...
rebound from embodied energy, substitution, and income effects.

Our approach has several important differences from the work of \Tt{} and \Bt{}.
We include embodied energy effects, 
whereas \Ba{} discussed but did not include them.
We incorporate differential capital costs between the original and upgraded devices,
whereas \Ta{} assumed they were equal.
We include operations and maintenance costs and disposal costs,
whereas neither \Ta{} nor \Ba{} included these costs.
...
% The contributions of this paper arise from enhancements to the previous work discussed above.
%
% \begin{enumerate}
% 
...
%         This is an advance from the work of \Bt{} who stopped short of deriving explicit, symbolic expressions.
%         Doing so enables both
% 
%           \begin{enumerate*}[label={(\alph*)}]
% 
%             \item identification of important drivers of overall energy rebound and
% 
%             \item explication of interactions among factors that affect overall energy rebound.
% 
%           \end{enumerate*}
% 
%   \item Further, we account for service life, operations and maintenance costs, and disposal costs explicitly.
%         These enhancements to the previous work of \Tt{} and \Bt{} and
%         allow us to assess the conditions under which
%         service life,
%         embodied energy,
%         capital costs,
%         operations and maintenence costs, and/or
%         disposal costs
%         will be important drivers of overall rebound.
% 
%   \item We include a macroeconomic multiplier ($k$) to describe technology changes.
%         Doing so allows quantification of the effects of technology changes on rebound.
%         **** Is this the right way to talk about $k$?
%         Adjust after seeing Gregor's inputs. ****
% 
% \end{enumerate}
%
...
The model enables, for the first time, via a case-study approach,
assessment of
the relative importance of several contributing factors
to the rebound effect.
And the model provides, for the first time,
a way to understand economy-wide rebound (via the macroeconomic multiplier)
...
We use, as our basis, the model of \Bt{}.
Like \Ba{}, we assume a device (say, an automobile)
...
with final-energy-to-service efficiency $\before{\eta}$ (in km/liter).
...
such that $\rateafter{C}_{dev} > \ratebefore{C}_{dev}$.\footnote{
  Following \Bt{},
...
% Our approach to capital costs is different from \Tt{p.~205},
% who assume the upgraded device to have the same
% capital cost as the original device, and
% \Bt{}, who does not address capital costs.
% Neither \Ta{} nor \Ba{} address operations and maintenance and disposal costs.
We spread capital and disposal costs across the life of the original and upgraded devices
($\before{t}$ and $\after{t}$, respectively)
to obtain cost rates
for the original and upgraded devices
($\ratebefore{C}_{dev}$ and $\rateafter{C}_{dev}$).%
\footnote{
  We choose not to apply time-value-of-money considerations to the financial aspects of the model.
  (\Bt{} discounted some cost streams in his examples.)
  ...
  (\Bt{} applied final-to-primary energy multipliers which divide out, anyway.)
  This decision simplifies derivations.
}

In our model,
the upgraded device may contain more (or less) embodied final energy
than the original device
such that $\after{E}_{\femb} \ne \before{E}_{\femb}$.%
% Our approach to embodied energy is different from \Tt{p.~205} who assume that
% the upgraded device has the same embodied energy as the original device,
% i.e., $\after{E}_{\femb} = \before{E}_{\femb}$.
% \Bt{} discusses embodied energy
% but does not include embodied energy effects in his model.%
\footnote{
  We define embodied energy as the energy/exergy analysis literature
  to be the sum of all final energy consumed in the production of the device.
  \Bt{} discusses ``embodied energy,'' but uses the term to mean
  the primary energy equivalent of direct final energy consumed.
}
Similar to device costs, 
we spread embodied energy across the expected life of the device to obtain a rate of embodied energy,
such that
$\ratebefore{E}_{\femb} = \before{E}_{\femb} / \before{t}$
and $\rateafter{E}_{\femb} = \after{E}_{\femb} / \after{t}$.

$U$ reduces the service price $p_s$ (in km/\$) such that $\after{p}_s < \before{p}_s$,
leading to substitution of spending
away from other purchases
toward final energy to operate the device.
The strength of the substitution dynamic is described
by the service price elasticity of final energy demand ($\epsilon$),
where typically $\epsilon < 0$.
Our approach to substitution effects is different from \Tt{}
who used input-output methods to
ascertain the energy cost of both substitution and income effects.
Our approach to service price elasticity of final energy demand is identical to \Bt{}.

The service price decline resulting from $U$
provides income (``freed cash'') to the device owner
to be spent elsewhere in the economy at the economy's final energy intensity
$I_{\Ef}$ (in MJ/\$).
Our approach to income effects is similar in practice to \Bt{}
who developed his model using a basket of $N$ other goods,
but who, in the numerical estimates, also assumed that freed cash has
energy implications at the average energy intensity of the economy.

Our derivations of
explicit, symbolic expressions for the four rebound effects
(device-level effects, embodied energy effects, substitution effects, and income and respending effects)
...
%
where
$\gamma \equiv \gammaequation{}$ is the
ratio of device cost rate increase 
to expected energy cost savings at the device level.
It is expected that $\gamma < 1$ 
such that $(1 - \gamma) > 0$,
...
thereby provides benefits to extending the work of \Bt{}
to obtain these expressions.
In particular, Equation~\ref{eq:Re_tot} allows us to
identify the relationships among total rebound and device-level rebound ($Re_{dev}$)
as well as many other factors.
The following subsections explore those relationships.


%++++++++++++++++++++++++++++++
\subsection{Embodied energy effects}
\label{sec:embodied_effects}
%++++++++++++++++++++++++++++++

Equation~\ref{eq:Re_tot} shows that
embodied energy rebound ($Re_{emb}$) contributes additively to total rebound
and is independent of device-level rebound.
When the embodied energy rate for a device
...
%++++++++++++++++++++++++++++++
\subsection{Device cost effects}
\label{sec:device_cost_effects}
%++++++++++++++++++++++++++++++

When costs for procuring, operating, maintaining, and disposing
the energy efficient device
($\rate{C}_{dev}$ and represented by $\gamma$ in Equations~\ref{eq:Re_ir} and~\ref{eq:Re_tot})
go up (down),
...
which boosts $Re_{ir}$ in Equation~\ref{eq:Re_ir}
...
Thus, device cost effects may lead to spending-related indirect energy rebound
...
for the original and upgraded devices
closes off the possibility of spending-related indirect rebound
due to device cost savings.
Furthermore, assuming identical capital costs
for the original and upgraded devices
...
When the capital cost for the upgraded device is larger than
the capital cost for the original device
($\rateafter{C}_{cap} < \ratebefore{C}_{cap}$),
...
Section~\ref{sec:income_effect_derivation}
shows that
operations and maintenance costs and disposal costs
can also affect respending-related indirect rebound.)


%++++++++++++++++++++++++++++++
\subsection{Device-level rebound effects}
\label{sec:device_level_rebound_effects}
%++++++++++++++++++++++++++++++

While the effects of embodied energy and device costs on total rebound
are straightforward and can be ascertained by inspection of Equation~\ref{eq:Re_tot},
the effect of device-level rebound ($Re_{dev}$) is less clear,
as $Re_{dev}$ appears in two terms with the opposite sense.
The first instance of $Re_{dev}$ (in the second term of the right side of Equation~\ref{eq:Re_tot})
enhances total rebound directly.
The second instance of $Re_{dev}$ (in the third term of the right side of Equation~\ref{eq:Re_tot})
reduces total rebound,
indicating that, as observed by \Tt{},
income effect rebound is inversely related to device-level rebound.
The inverse relationship arises because energy cost savings taken back by device-level rebound
cannot be spent in the economy, thereby reducing rebound from the income effect.
Which effect dominates on net is shown by Equation~\ref{eq:Re_sub}:
device-level rebound
enhances (reduces) total rebound when the product $I_{\Ef} p_{\Ef}$ is less than (greater than) 1.
Typically, $I_{\Ef} p_{\Ef} < 1$, as shown in Section~\ref{sec:examples}, 
so device-level rebound typically enhances total rebound.


%++++++++++++++++++++++++++++++
\subsection{Elasticity effects}
\label{sec:elasticity effects}
%++++++++++++++++++++++++++++++

Similar to device-level rebound, 
the effect of service price elasticity of final energy demand ($\epsilon$)
on total rebound ($Re$) is 
confounded by the relationship between the product $I_{\Ef} p_{\Ef}$ and 1.
But under typical conditions%
\footnote{
  ...
  \begin{enumerate*}[label={(\alph*)}]
    ...
    \item $U$ increases final-energy-to-service efficiency ($\after{\eta} > \before{\eta}$), and
  
    \item $0 < p_{\Ef} I_{\Ef} < 1$ (see Section~\ref{sec:examples} below).

  \end{enumerate*}
},
a more-negative (less-negative) service price elasticity of final energy demand
($\epsilon$) leads to larger (smaller) rebound.


%++++++++++++++++++++++++++++++
\subsection{Multiplier, energy intensity, and energy price effects}
\label{sec:k_I_p_effects}
%++++++++++++++++++++++++++++++
...
\begin{enumerate*}[label={(\alph*)}]
  ...
  \item higher energy prices ($p_{\Ef}$) as greater energy cost savings from $U$ are spent in the economy, and

  \item higher final energy intensity of the economy ($I_{\Ef}$) as respent energy cost savings demand additional energy in the economy.
...
$1 - Re_{dev}/k - \gamma$ will remain positive. 
If device-level rebound is so large that 
$1 - Re_{dev}/k - \gamma < 0$,
total energy rebound will be reduced by a larger multiplier ($k$),
...
As \Bt{} notes, a good first estimate for $Re_i$ 
is obtained when
%
\begin{enumerate*}[label={(\alph*)}]

  \item $U$ is cost-neutral with no effect on device costs ($\rateafter{C}_{dev} = \ratebefore{C}_{dev}$, leading to $\gamma = 0$) and
...
simplifies to $Re_i = I_{\Ef} p_{\Ef}$.
...
The average price of gasoline in the US in 2017 was
...
a quick estimate for income effect rebound is
$Re_i = p_{\Ef} I_{\Ef}$ =
\Sexpr{format(pI_gasoline, digits = 3)} or 
\Sexpr{format(pI_gasoline * 100, digits = 3)} \%.

The averge price of electricity in the US in 2017 was
...
a quick estimate for income effect rebound is
$Re_i = p_{\Ef} I_{\Ef}$ =
\Sexpr{format(pI_electricity, digits = 3)} or
\Sexpr{format(pI_electricity * 100, digits = 3)} \%.

Income \emph{and} respending rebound is found by multiplying by $k$ such that $Re_{ir} = k Re_i$.
...
\begin{tabular}{r l}
  ...
  $C$ & cost [\$] \\
  $E$ & energy [MJ] \\
  $e$ & inverse of device efficiency ($\eta$); used by \Bt{}. \\
  $FC$ & freed cash [\$] \\
  $G$ & income [\$] \\
...
Expected energy savings are often called \emph{potential} energy savings.
Prior to the energy efficiency upgrade~($U$),
the device has final-energy-to-service efficiency of $\before{\eta}$, and
...
when assessing expected energy savings.
...
\begin{equation} \label{eq:rebound_2}
  Re_i = \frac{\mathrm{takeback}_i}{\left(1 - \frac{\before{\eta}}{\after{\eta}} \right) \ratebefore{E}_{\fdir}} \, .
\end{equation}
...
$U$ causes the embodied final energy of the device to change
...
When a device is upgraded ($U$) to higher efficiency ($\after{\eta} > \before{\eta} > 0$),
...
%
\begin{equation}
  Re_{dev} = \Redevequation{} \, . \tag{\ref{eq:Re_dev}}
\end{equation}
...
such that $\rate{C}_{\shift} = \rate{E}_{\fgain} p_{\Ef}$.
Assuming that $\rate{C}_{\shift}$ reduces energy consumption
by the energy intensity of the economy gives
$\rate{E}_{\floss} = \rate{C}_{\shift} I_{\Ef} = \rate{E}_{\fgain} p_{\Ef} I_{\Ef}$.
...
\begin{equation} \label{eq:delta_sub}
  \Delta \rate{E}_{\fsub} = (1 - p_{\Ef} I_{\Ef})
...
$Re_{sub} = (1 - p_{\Ef} I_{\Ef}) \left[ \left( \frac{\before{\eta}}{\after{\eta}} \right)^\epsilon  - 1 \right]
...
$\rate{G}$ is money that is no longer spent on energy to operate the upgraded device,
...
If capital and disposal costs are spread across the life of the device, we obtain
...
\footnote{
  Note that neither $\ratebefore{C}_{OM}$ nor $\rateafter{C}_{OM}$ include direct energy costs
...
``akin to the macroeconomic multiplier that might result
following a new discovery of a natural resource deposit'' \Bp{p.~11}.
The effect of this respending can be represented by a technology multiplier~($k$).
Although \Ba{} did not include a multiplier ($k$) in his model,
we do so here.\footnote{
  Not including the multiplier, in effect, assumes $k = 1$.
}
Thus, the change in final energy consumption due to income and respending can be written as
$\Delta \rate{E}_{\fincres} = FC \, I_{\Ef} k$.
...
\begin{equation} \label{eq:delta_income}
  \Delta \rate{E}_{\fincres} = \left( 1 - \frac{\before{\eta}}{\after{\eta}}
                               ...
                               k I_{\Ef} p_{\Ef} \ratebefore{E}_{\fdir} \, .
\end{equation}
...
Similarly, $\Delta \rate{E}_{\fincres}$ goes up when
...
\end{equation}
%
Note that rebound from income only ($Re_i$) is obtained from Equation~\ref{eq:Re_ir} with $k = 1$,
...
economic parameters ($k$, $I_{\Ef}$, and $p_{\Ef}$).
%
\begin{equation}
...
\Bt{pp.~15--17} provides
example calculations for rebound caused by increasing auto fuel economy.
The details of \Ba{}'s
...
Agreement will indicate that the derivation of Equation~\ref{eq:Re_ir} is correct.
