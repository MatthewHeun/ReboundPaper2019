...
$\Delta \rate{E}_{\fincres}$ goes up.
...
\begin{equation} \label{eq:rebound_total}
  Re = Re_{emb} + Re_{dev} + \left( 1 - \frac{Re_{dev}}{k} - \gamma \right) k I_{E_f} p_{E_f} \, .
\end{equation}
...
\citet[Section IV.A]{Borenstein:2015aa} provides
example calculations for rebound from an increase in auto fuel economy.
...
($Re_{inc}$ and $Re_{sub}$) in Equations~\ref{eq:Re_sub} and~\ref{eq:Re_inc} 
...
%------------------------------
\subsubsection{Verification of substitution effect rebound ($Re_{sub}$) equation} 
...
%------------------------------
\subsubsection{Verification of income effect rebound ($Re_{inc}$) equation} 
...
Then, we compare our answer to 
\citeauthor{Borenstein:2015aa}'s numerical estimate for $Re_{inc}$.
Agreement will indicate that the derivation of Equation~\ref{eq:Re_inc} is correct.

The explicit equation for income effect rebound ($Re_{inc}$) is given by
...
($\ratebefore{E}_{\fdir}$)
is calculated from expected usage (12,000 mi/yr) and $\eta$ as
...
we obtain $Re_{inc} = 0.110$, 
exactly matching the value of 0.11 reported in \citet[Section IV.A]{Borenstein:2015aa}.
