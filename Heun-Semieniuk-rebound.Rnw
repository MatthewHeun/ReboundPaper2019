...
\newcommand{\finc}{\fcomma{}inc}                   % Improves spacing between f,inc
\newcommand{\incres}{inc\&res}                     % Subscript for income and respending
...
the rate of final energy consumption directly by the device~($\rate{E}_{\fdir}$) by
...
and more embodied energy ($\after{E}_{\femb} > \before{E}_{\femb}$). 
...
\begin{equation} \label{eq:Re_emb}
  Re_{emb} = \frac{\left( \frac{\after{E}_{\femb}}{\before{E}_{\femb}}\frac{\before{t}}{\after{t}} - 1 \right)}
             {\left( 1 - \frac{\before{\eta}}{\after{\eta}} \right)}
             \frac{\ratebefore{E}_{\femb}}{\ratebefore{E}_{\fdir}} \, .
\end{equation}
%
Embodied energy rebound increases to the extent that
%
\begin{enumerate*}[label={(\alph*)}]
	
  \item the original embodied energy rate is larger than the original direct final energy consumption rate 
        ($\ratebefore{E}_{\femb} > \ratebefore{E}_{\fdir}$) or 
  
  \item the embodied energy of the device is larger after $U$ than before
        ($\after{E}_{\femb} > \before{E}_{\femb}$), 
...
As such, $\rate{E}_{\fgain}$ is the source of device-level rebound. 
...
After simplification, the explicit expression for device-level rebound is
%
...
$Re_{sub} = (1 - p_{E_f} I_{E_f}) \left[ \left( \frac{\before{\eta}}{\after{\eta}} \right)^\epsilon  - 1 \right] / \left[ \left( \frac{\after{\eta}}{\before{\eta}} \right) - 1 \right]$. 
After simplification, substitution rebound can be expressed as
%
\begin{equation} \label{eq:Re_sub}
  Re_{sub} = (1 - p_{E_f} I_{E_f}) Re_{dev} \, .
\end{equation}
...
Energy cost savings~($\rate{G}$) (known as ``income''\footnote{
...
$\rate{G}$ is money that is no longer spent on energy, 
assuming the same service level from the device. 
Thus, $\rate{G} = -\Delta \rate{E}_{\fdir} p_{E_f}$. 
Because $\Delta \rate{E}_{\fdir}$ is negative, $\rate{G}$ is positive.
It costs money to procure, operate, maintain, and dispose of the energy-efficient device.
But it also cost money to procure, operate, maintain, and dispose of the original device, too.
If costs are spread across the life of the device, we obtain
$\rateafter{C}_{dev} = \after{C}_{pur} / \after{t} + \rateafter{C}_{OM} + \after{C}_{disp} / t$ and
...
But the freed cash is respent in the economy.
The respending can be represented by a technology multiplier~($k$).
Thus, 
$\Delta E_{\fincres} = FC \, I_{E_f} k$.
Using the definitions above yields
%
\begin{equation} \label{eq:deta_income}
...
As $\after{\eta}$ increases, $\Delta \rate{E}_{\fincres}$ goes up.
More freed cash is available when $\rateafter{C}_{dev} > \ratebefore{C}_{dev}$, and
$\Delta \rate{E}_{\fdir}$ goes up.
...
$\gamma \equiv \left( \frac{1}{1 - \frac{\before{\eta}}{\after{\eta}}} \right) 
                       \frac{\rateafter{C}_{dev} - \ratebefore{C}_{dev}}{\ratebefore{E}_{\fdir} p_{E_f}}$
to represent the effect of device costs,
income effect rebound can be written as
%
\begin{equation} \label{eq:Re_inc}
  Re_{\incres} = ( 1 - \gamma) k I_{E_f} p_{E_f} \, .
\end{equation}
%
As $\rateafter{C}_{dev} - \ratebefore{C}_{dev}$ goes up, $Re_{\incres}$ goes down,
because the cost of procuring, operating, maintaining, and disposing of the device 
eats away at the energy cost savings available to be respent in the economy.
If $\rateafter{C}_{dev} = \ratebefore{C}_{dev}$, $U$ is costless, and 
...
Total rebound is given by $Re = Re_{emb} + Re_{sub} + Re_{\incres}$.
After substituting and rearranging, total rebound is found to be
%
\begin{equation} \label{eq:rebound_total}
  Re = Re_{dev} + Re_{emb} + \left( 1 - \frac{Re_{dev}}{k} - \gamma \right) k I_{E_f} p_{E_f} \, .
\end{equation}
%
