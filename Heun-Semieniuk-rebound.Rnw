...
\citet{Thomas:2013aa, Thomas:2013ab} look at household spending
to ascertain embodied energy effects
of spending energy cost savings in the economy.
...
he did not develop an expression for it,
nor did he make estimates of its magnitude.
He mentioned a ``macroeconomic multiplier'' but did not include it
...
In contrast to \Tt{},
we account for potential differences in embodied energy between the original and upgraded device.





We spread embodied energy and initial and disposal costs
        across the service life of the device,
        thereby assuming that the analysis applies for a ``typical year''
        of the device.
        (\Bt{} didn’t account for varying service life
...
We choose not to apply time-value-of-money considerations to the financial aspects of the model.
        (\Ba{} discounted some cost streams in his examples.)
...
In this paper, we extend the work of \Tt{} and \Bt{} in several important ways.
%
\begin{enumerate}
  ...
  \item We develop explicit, symbolic rebound expressions for four effects on rebound:
        ...
        Doing so enables both indentification of important factors for rebound and
        explication of interactions among factors that affect rebound.
  ...
  \item We include embodied energy effects in the rebound model.
        We then estimate the magnitude of embodied energy effects and compare to other factors to ascertain
...
to provide a rate of energy service $\ratebefore{q}_s$ (in vehicle miles/year)
with final-energy-to-services efficiency $\before{\eta}$ (in miles/gallon).
An energy efficiency upgrade ($U$) increases
final-energy-to-services efficiency such that
...
to obtain cost rates ($\ratebefore{C}_{dev}$ and $\rateafter{C}_{dev}$).
...
$U$ reduces the service price $p_s$ (in miles/\$) such that $\after{p}_s < \before{p}_s$, 
...
thereby provides a benefit of extending the work of \Bt{}
...
($\rate{C}_{dev}$ and represented by $\gamma$ in Equation~\ref{eq:Re_ir})
go up (down),
...
Like \Bt{}, we consider three contributing factors to rebound,
