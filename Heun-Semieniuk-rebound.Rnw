...
We include differential operations and maintenance costs and disposal costs
...
The new model enables, via a case-study approach,
assessment of
...
the next section extends the \Bt{} model 
to develop our new, unified, simple-to-understand, clearly derived model
of direct and indirect rebound.
We also explicate the new model, demonstrating 
it can be used to elucidate various rebound effects and trends.
...
\footnote{
  ...
  (\Bt{} applied final-to-primary energy multipliers which divide out in rebound expressions, anyway.)
...
but who, in his numeric estimates, also assumed that freed cash has
...
\end{equation}
%
Embodied energy rebound is one mechanism for indirect rebound.
...
\end{equation}
%
The term $p_{\Ef} I_{\Ef}$ represents 
an energy shift away from purchases in the rest of the economy,
...
$k$ is a macroeconomic multiplier. 
**** Discussion here about $k$ to be supplied by Gregor. ****

$Re_i$ is an indirect rebound term.
To the extent that $k > 1$, income and respending rebound is a macroeconomic effect.
Thus, $Re_{ir}$
...
($\rate{C}_{dev}$, represented by $\gamma$ in Equations~\ref{eq:Re_ir} and~\ref{eq:Re_tot})
go up (down),
...
simplifies to $Re_i = p_{\Ef} I_{\Ef} $.
...
It is instructive to calculate the product $p_{\Ef} I_{\Ef}$ under typical conditions.
The average energy intensity of the economy ($I_{\Ef}$) is independent of energy carrier, 
while price ($p_{\Ef}$) varies by energy carrier.
