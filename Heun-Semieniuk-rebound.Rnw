...
Although we are guided by the \citet{Borenstein:2015aa}, 
our approach differs from \citeauthor{Borenstein:2015aa} in several important ways.

**** focus on EXTENDING **** where possible.
...
\begin{enumerate}
  ...
  \item We spread embodied energy and initial and disposal costs
        ...
        thereby assuming that the analysis applies to a ``typical year''
        of the device.
        This approach allows for service life differences between the initial and upgraded devices.
        (\citeauthor{Borenstein:2015aa} didn’t account for varying service life
        in his theory and equations.
        Service life was applied in example calculations.
        Our approach makes service life an explicit aspect of the equations.)

  \item We do not apply time-value-of-money considerations to the financial aspects of the model.
        (\citeauthor{Borenstein:2015aa} discounted some cost streams in his examples.)
  
  \item We do not apply final-to-primary energy multipliers to final energy rates, because
        these multipliers appear in both numerator and denominator of rebound expressions.
        (\citeauthor{Borenstein:2015aa} applied multiplier which, ultimately divided out.)
  
  \item We add a multiplier ($k$) to describe technology changes.
        (\citet[p.~11--13]{Borenstein:2015aa} discussed a multiplier but did not include 
        a multiplier in his income-effect calculations, effectively assuming $k = 1$..) 

\end{enumerate}

With these modifications to the \citeauthor{Borenstein:2015aa} approach,
we derive an explicit, symbolic version of the \citet{Borenstein:2015aa} model
by analyzing four mechanisms of energy rebound: 
device-level effects, embodied energy effects, 
substitution effects, and income and respending effects.
(See the Appendix, Section~\ref{sec:model_derivation}, for details.)
...
%
where
a symbol (e.g., $\before{X}$ without decoration) indicates $X$ before the energy efficiency upgrade,
$\after{X}$ indicates $X$ after $U$, 
$\eta$ is final-energy-to-services efficiency before $U$ ($\before{\eta}$) and after $U$ ($\after{\eta}$), and
...
Income and respending effect rebound ($Re_{ir}$) is given by 
%
...
the economic expansion casued by $U$.
...
thereby represents an advance from the work of \citeauthor{Borenstein:2015aa}.
...
device costs reduce (increase) freed cash available 
for spending in the economy.

While the effects of embodied energy and device costs on total rebound are clear,
the effect of device-level rebound ($Re_{dev}$) is less clear, 
as $Re_{dev}$ appears in two terms of Equation~\ref{eq:Re_tot}.
However, Equation~\ref{eq:Re_sub} shows that device-level rebound 
enhances total rebound when $I_{E_f} p_{E_f} < 1$ and 
reduces total rebound when $I_{E_f} p_{E_f} > 1$.

The third term of Equation~\ref{eq:Re_tot} shows that 
when $1 - Re_{dev}/k - \gamma > 0$, 
total energy rebound is enhanced by 
larger multiplier ($k$) due to enhanced respending in the economy,
higher energy prices ($p_{E_f}$) as energy cost savings are spent in the economy, and
higher final energy intensity of the economy ($I_{E_f}$) as respent energy cost savings demand additional energy in the economy.
...
When $U$ 
implies no additional embodied energy ($Re_{emb} = 0$), 
has no device-level rebound ($Re_{dev} = 0$), and 
is costless ($\gamma = 0$), 
total final energy rebound is simply $k I_{E_f} p_{E_f}$.
