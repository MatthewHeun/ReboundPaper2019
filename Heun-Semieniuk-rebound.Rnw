...
**** Pull final equation up to main text and discuss,  
especially the effect of $Re_{dev}$. 
$Re_{emb}$ directly contributes to total rebound. 
$Re_{dev}$ is less clear and can enhance or diminish total rebound. 
Device costs (expressed as $\gamma$ reduce rebound. ****
...
such that $\before{\eta} = 1/e$ and $\after{\eta} = 1/\tilde{e}$.
...
After substituting and rearranging, total rebound can be expressed in terms of
embodied energy rebound~($Re_{emb}$),
device-level rebound~($Re_{dev}$), 
the nondimensional change in device cost rate~($\gamma$), and
