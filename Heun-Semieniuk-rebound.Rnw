...
% Income rebound
\newcommand{\Reiequation}{( 1 - \gamma) I_{E_f} p_{E_f}}
...
\newcommand{\Ba}{\citeauthor{Borenstein:2015aa}}
\newcommand{\Bp}{\citep{Borenstein:2015aa}}
\newcommand{\Bt}{\citet{Borenstein:2015aa}}
...
This statement\footnote{Not actually a statement} needs referencing \Bp{}. 
...
The landmark work of \Bt{} 

Neither \Bt{} mentioned embodied energy, but 
...
In this paper, we extend the \Bt{} model in several important ways.
%
\begin{enumerate}
...
Like \Bt{}, we assume that 
a device (say, an automobile) consumes final energy ($E_f$, say gasoline) 
to provide energy services (vehicle miles) 
at rate $\ratebefore{q}_s$ (in miles/year)
...
at an increased financial cost to procure, operate, maintain, and dispose the device
...
The upgraded device may also contain more embodied final energy
than the original
such that $\after{E}_{\femb} > \before{E}_{\femb}$.
$U$ reduces the service price such that $\after{p}_s < \before{p}_s$, 
freeing cash to be spent elsewhere 
at the energy intensity of the broader economy ($I_{E_f}$).
...
which is typically less than 0.
...
using data from \Ba{}'s auto efficiency example.)
