...
Project_data <- Project_data %>%
  dplyr::mutate(
    ...
    class = factor(class, levels = c("backfire", "rebound", "hyperconservation"))
  )
@
...
Project_data %>% 
  ...
  tri_labels(size = 6, padding = 0.05, labels = c(x = "phi[device]", y = "phi[emb]", z = "phi[respend]"), colour = "black") +
  geom_point(aes(x=phi_Re_dev, y=phi_epsilon, z=phi_respend, fill = class), stat="triangle", 
