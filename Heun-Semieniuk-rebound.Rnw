...
\Tt{} provide a helpful summary of direct rebound estimates,
...
\Bt{} developed expressions for changes in final energy demand
due to substitution and income effects,
and he calculated, numerically, estimates for substitution and income rebound
...
in his expressions or numerical estimates.
...
Our approach has important differences from the work of \Tt{} and \Bt{}.
\Tt{} included embodied energy effects via input-output analysis,
while \Bt{} discussed embodied energy but did not include it in his model or calculations.
\Bt{} incorporated capital costs in his calculations via service life differences, while
\Tt{} assumed no change in capital costs between the original and upgraded devices.
Neither \Tt{} and \Bt{} considered differences in operations and maintenance costs or disposal costs
between the original and upgraded devices.
We do incorporate a macro
...
The contributions of this paper arise from enhancements to the previous work
of \Tt{} and \Bt{}.
%
\begin{enumerate}
  ...
  \item Further, we account for service life, operations and maintenance costs, and disposal costs explicitly.
        ...
        allows us to assess the conditions under which
        service life,
        embodied energy,
        capital costs,
...
based on microeconomic and energy fundamentals.
The model enables, for the first time, assessment of
the relative contributions to direct and indirect rebound
of several important factors.
The model provides, for the first time,
a way to understand economy-wide rebound
...
procure (capital cost), operate, maintain, and dispose the device
...
Neither \Ta{} nor \Ba{} address disposal costs.
...
provides income to the device owner
...
who ascertained energy costs from a basket of a large number of goods,
but who, in the end, also assumed that freed cash has
energy implications at the energy intensity of the economy.
...
%
where
$\gamma \equiv \gammaequation{}$ is the nondimensionalized change in device cost rate.
...
Thus, device cost effects may lead to respending-related indirect energy rebound
...
for the original and upgraded devices
closes off the possibility of respending-related indirect rebound
...
the capital cost for the original device
($\rateafter{C}_{cap} < \ratebefore{C}_{cap}$),
assuming equal capital costs will tend to underestimate $Re$.
...
cannot be spent in the economy, thereby reducing rebound from the inccome effect.
Which effect dominates is shown by Equation~\ref{eq:Re_sub}:
device-level rebound
enhances (reduces) total rebound when $I_{\Ef} p_{\Ef}$ is less than (greater than) 1.
...
\end{enumerate*}
%
When $I_{\Ef} p_{\Ef}$ is less than (greater than) 1,
higher service price elasticity of demand (a more-negative $\epsilon$)
will lead to larger (smaller) energy rebound,
because $Re_{dev}$ gets more positive as $\epsilon$ gets more negative and $1 - p_{\Ef} I_{\Ef}$ is positive (negative).
...
Furthermore, we choose not to apply final-to-primary energy multipliers
to final energy rates in the numerators and denominators of rebound expressions.
(\Ba{} applied final-to-primary energy multipliers which divide out, anyway.)
