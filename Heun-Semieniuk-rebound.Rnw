...
\newcommand{\after}[1]{\widetilde{#1}}
\newcommand{\rate}[1]{\dot{#1}}
\newcommand{\rateafter}[1]{\widetilde{\dot{#1}}}
...
\begin{tabular}{r l}
  ...
  \midrule
  $X$ & (without decoration) prior to energy efficiency intervention \\
  $\after{X}$ & after energy efficiency intervention \\
  $\rate{X}$ & rate of $X$ before energy efficiency intervention [X/year] \\
  $\rateafter{X}$ & rate of $X$ after energy efficiency intervention [X/year] \\
  \bottomrule
\end{tabular}
...
device direct energy consumption~($E_{f,dir}$) by
...
the device has final energy-to-services efficiency of $\eta$, and
the user of the energy conversion device consumes services 
provided by the device at a rate of $\rate{q}_s$, 
which implies the final energy consumption rate of the device to be
$\rate{E}_{f,dir} = \rate{q}_s / \eta$.
After $U$, the rate of final energy consumption by the device is
$\rateafter{E}_{f,dir} = \rate{q}_s / \after{\eta}$, 
because the rate of service consumption is assumed to be unchanged 
(i.e., $\rateafter{q}_s = \rate{q}_s$).
The change in the direct energy consumption rate of the device~($\Delta \rate{E}_{f,dir}$) 
due to $U$ is given by 
$\Delta \rate{E}_{f,dir} = \frac{\rate{q}_s}{\after{\eta}} - \frac{\rate{q}_s}{\eta}$, 
which can be rearranged to be
%
\begin{equation}
  \Delta \rate{E}_{f,dir} = \left( \frac{\eta }{\after{\eta}} - 1 \right) \rate{E}_{f,dir}
\end{equation}
%
using $\rate{E}_{f,dir} = \frac{\rate{q}_s}{\eta}$.
If $\after{\eta} \rightarrow \infty$, 
the infinite efficiency eliminates all final energy consumption, and 
$\Delta \rate{E}_{f,dir} = - \rate{E}_{f,dir}$, as expected.
If efficiency increases due to $U$ such that $\after{\eta} > \eta$, 
...
or $-\Delta \rate{E}_{f,dir} = (1 - \eta / \after{\eta}) \rate{E}_{f,dir}$.
Thus Equation~\ref{eq:rebound_1} can be rewritten as, 
\begin{equation} \label{eq:rebound_2}
  Re_i = \frac{\mathrm{takeback_i}}{\left(1 - \frac{\eta}{\after{\eta}} \right) \rate{E}_{f,dir}} \, .
\end{equation}
...
We allocate embodied energy over the life of the device ($t$ and $\after{t}$) 
...
\begin{equation} \label{eq:delta_embodied}
  \Delta \rate{E}_{f,emb} = \left( \frac{\after{E}_{f,emb}}{E_{f,emb}}
  \frac{t}{\after{t}} - 1 \right) \rate{E}_{f,emb} \, .
\end{equation}
%
Typically, increases in energy efficiency are associated with increased device complexity
and more embodied energy ($\after{E}_{f,emb} > E_{f,emb}$). 
...
\begin{equation} \label{eq:Re_emb}
  Re_{emb} = \frac{\left( \frac{\after{E}_{f,emb}}{E_{f,emb}}\frac{t}{\after{t}} - 1 \right)}
             {\left( 1 - \frac{\eta}{\after{\eta}} \right)}
             \frac{\rate{E}_{f,emb}}{\rate{E}_{f,dir}} \, .
\end{equation}
%
As the original embodied energy rate is larger than the direct final energy consumption rate 
($\rate{E}_{f,emb} > \rate{E}_{f,dir}$), 
embodied energy rebound will be larger.
As the embodied energy of the device is larger after $U$ than before
($\after{E}_{f,emb} > E_{f,emb}$), 
embodied energy rebound will be larger.
If the upgraded device has longer life than the original device
($\after{t} > t$), 
embodied energy rebound will be smaller.
To the extent that $U$ increases efficiency
($\after{\eta} > \eta$),
embodied energy will be smaller.
...
%------------------------------
\subsubsection{Device-level rebound} 
\label{sec:device_level_rebound_derivation}
%------------------------------
