...
which implies final energy consumption by the device at a rate 
...
$\Delta \rate{E}_{f,dir} = - \Delta  \rate{E}_{f,dir}$, as expected.
If efficiency increases such that $\after{\eta} > \eta$, 
...
Embodied energy rebound is given by the ratio of embodied energy takeback to expected, 
device-level energy savings due to $U$.
%
