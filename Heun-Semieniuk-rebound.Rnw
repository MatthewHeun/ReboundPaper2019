...
\begin{tabular}{r l}
  ...
  Subscript & Meaning \\
  \midrule
  $dir$ & direct (denotes energy used directly by the device) \\
  $dev$ & device \\
  $E_f$ & final energy \\
  $emb$ & embodied \\
  $s$ & service \\
...
The change in the direct energy consumption rate of the device~($\Delta \rate{E}_{f,dir}$) is given by 
...
or $(1 - \eta / \after{\eta}) \rate{E}_{f,dir}$.
