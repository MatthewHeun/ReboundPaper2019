...
with final-energy-to-services efficiency $\before{\eta}$ (miles/gallon).
...
procure (capital), operate, maintain, and dispose the device
...
$U$ reduces the service price (in, say, miles/\$) such that $\after{p}_s < \before{p}_s$, 
freeing cash to be spent elsewhere in the economy at its final energy intensity
($I_{\Ef}$, in MJ/\$).
Our approach to freed cash is different from \Tt{} who used input-output methods 
to ascertain the energy cost of income and substitution effects. 
Our approach to freed cash is similar in practice to \Bt{} 
who discussed ascertaining energy costs from a basket of a large number of goods, 
but who, in the end, also assumed that freed cash has 
energy implications at the energy intensity of the economy.

The reduced service price may also lead to substitution of spending
