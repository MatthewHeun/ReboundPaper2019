...
Project_data %>% 
  dplyr::filter(Source != "Heun" & MPC == 0.95) %>%
  ggplot() +
  tri_theme() +
...
The following graphs are used for the ENGR333 presentations.

<<Empty_Ternary_Graph, fig.width=3.5, fig.height=2.25>>=
Project_data %>% 
  dplyr::filter(Source != "Heun" & MPC == 0.95) %>%
  ...
  tri_theme() +
  tri_grid(colour="black") +
  tri_labels(size=6, padding=.05, labels = c(x = "phi[device]", y = "phi[embodied]", z = "phi[respend]"), colour = "black") +
  geom_point(aes(x=phi_Re_dev, y=phi_epsilon, z=phi_respend), stat="triangle", size = 0, alpha = 1) + 
  theme(plot.margin = margin(0, 21, 0, 12)) +
  coord_cartesian(clip = "off")
@

<<Ternary_graph_all, fig.width=6.5, fig.height=5>>=
Project_data %>% 
  dplyr::filter(Source != "Heun", MPC == 0.95) %>% 
  ggplot() +
  tri_theme() +
  tri_grid(colour = "black") +
  tri_labels(size = 6, padding = 0.05, labels = c(x = "phi[device]", y = "phi[embodied]", z = "phi[respend]"), colour = "black") +
  geom_point(aes(x=phi_Re_dev, y=phi_epsilon, z=phi_respend, fill = rebound_type), stat="triangle", 
             size = 6, alpha = 1, shape = 21, colour = "black") +
  scale_fill_manual(values = scales::hue_pal()(3) %>% set_names("backfire", "hyperconservation", "rebound")) + 
  theme(legend.title=element_blank(), legend.text = element_text(colour = "black"), plot.margin = margin(0, 0, 0, 12)) + 
  coord_cartesian(clip = "off")
@


<<Ternary_graph_autos, fig.width=6.5, fig.height=3.5>>=
Project_data %>% 
  dplyr::filter(Source != "Heun" & startsWith(Example, "Autos"), MPC == 0.95) %>% 
  ggplot() +
  tri_theme() +
  tri_grid(colour = "black") +
  tri_labels(size = 6, padding = 0.05, labels = c(x = "phi[device]", y = "phi[embodied]", z = "phi[respend]"), colour = "black") +
...
<<Ternary_Graph_hyperconservation, fig.width=3.5, fig.height=2.5>>=
Project_data %>% 
  dplyr::filter(Source != "Heun" & MPC == 0.95) %>%
  dplyr::filter(rebound_type == "hyperconservation") %>% 
  ggplot() +
  tri_theme() +
  tri_grid(colour="black") +
  tri_labels(size=6, padding=.05, labels = c(x = "phi[device]", y = "phi[embodied]", z = "phi[respend]"), colour = "black") +
  geom_point(aes(x=phi_Re_dev, y=phi_epsilon, z=phi_respend, fill = rebound_type), stat="triangle", 
             size = 6, alpha = 1, shape = 21, colour = "black", show.legend = FALSE) +
  scale_fill_manual(values = scales::hue_pal()(3) %>% set_names("backfire", "hyperconservation", "rebound")) + 
  theme(legend.title=element_blank(), legend.text = element_text(colour = "black"), plot.margin = margin(2, 21, 0, 12))+ 
  coord_cartesian(clip = "off")
@

<<Ternary_Graph_rebound, fig.width=3.5, fig.height=3.5>>=
Project_data %>% 
  dplyr::filter(Source != "Heun" & MPC == 0.95) %>%
  dplyr::filter(rebound_type == "rebound") %>% 
  ggplot() +
  tri_theme() +
  tri_grid(colour="black") +
  tri_labels(size=6, padding=.05, labels = c(x = "phi[device]", y = "phi[embodied]", z = "phi[respend]"), colour = "black") +
  geom_point(aes(x=phi_Re_dev, y=phi_epsilon, z=phi_respend, fill = rebound_type), stat="triangle", 
             size = 6, alpha = 1, shape = 21, colour = "black", show.legend = FALSE) +
  scale_fill_manual(values = scales::hue_pal()(3) %>% set_names("backfire", "hyperconservation", "rebound")) + 
  theme(legend.title=element_blank(), legend.text = element_text(colour = "black"), plot.margin = margin(0, 22, 0, 12))+ 
  coord_cartesian(clip = "off")
@

<<Ternary_Graph_backfire, fig.width=3.5, fig.height=2.5>>=
Project_data %>% 
  dplyr::filter(Source != "Heun" & MPC == 0.95) %>%
  dplyr::filter(rebound_type == "backfire") %>% 
  ggplot() +
  tri_theme() +
  tri_grid(colour="black") +
  tri_labels(size=6, padding=.05, labels = c(x = "phi[device]", y = "phi[embodied]", z = "phi[respend]"), colour = "black") +
  ...
  theme(legend.title = element_blank(), 
        ...
        plot.margin = margin(0, 22, 0, 12)) + 
  coord_cartesian(clip = "off")
@


<<Ternary_graph_sensitivity, fig.width=6.5, fig.height=5.75>>=
Project_data_filtered <- Project_data %>% 
  dplyr::filter(Source != "Heun") %>% 
  dplyr::filter(phi_respend <= 2, phi_epsilon >= -2, phi_epsilon < 2)
# Project_data_removed <- dplyr::anti_join(Project_data, Project_data_filtered)
Project_data_filtered %>% 
  dplyr::mutate(
    MPC = factor(MPC, levels = c(0.95, 0.9, 0.85, 0.75))
  ) %>% 
  ggplot() +
  tri_theme() +
  tri_grid(colour = "black") +
  tri_labels(size = 4, padding = 0.05, labels = c(x = "phi[device]", y = "phi[embodied]", z = "phi[respend]"), colour = "black") +
  geom_point(aes(x=phi_Re_dev, y=phi_epsilon, z=phi_respend, fill = rebound_type), stat="triangle", 
             size = 3, alpha = 1, shape = 21, colour = "black") +
  scale_fill_manual(values = scales::hue_pal()(3) %>% set_names("backfire", "hyperconservation", "rebound")) + 
  theme(legend.title=element_blank(), 
        legend.text = element_text(colour = "black"), 
