...
$I_{\Ef}$ (in MJ/\$)%
\footnote{
  In 2017, total final energy consumption in the US 
  was 1521 Mtoe = 63.6812 EJ = $63.6812 \times 10^{12}$ MJ~\citep{IEA2020:sankey, IEA2020:units}, and 
  the size of the US economy was 19,519,424 M\$ or $\$19.519 \times 10^{12}$~\citep{BEA:2020}.
  Thus, the final energy intensity of the US economy was $I_{\Ef} = 3.258$ MJ/\$.
}.
...
%
where
$\gamma \equiv \gammaequation{}$ is the nondimensionalized change in device cost rate, and
$\ratebefore{E}_{\fdir}$ is the direct energy consumption rate of the device 
prior to the energy efficienc upgrade.
...
\begin{enumerate*}[label={(\alph*)}]
  ...
  \item $U$ increases final-energy-to-service efficiency ($\after{\eta} > \before{\eta}$),
  
  \item $U$ is economic for the device owner ($p_{\Ef} I_{\Ef} < 1$), and

  \item $\epsilon < 0$ such that
        the increase in final-energy-to-service efficiency
        leads to a reduction in service price ($\after{p}_s < \before{p}_s$),
        additional device usage, and $Re_{dev} > 0$.

\end{enumerate*}
%
Under these conditions, a more-negative (less-negative) 
service price elasticity of final energy demand ($\epsilon$)
will lead to larger (smaller) rebound.
...
Total final energy rebound simplifies to $k I_{\Ef} p_{\Ef}$ if
%
\begin{enumerate*}[label={(\alph*)}]
