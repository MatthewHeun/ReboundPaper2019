...
In this paper, we extend the work of \Bt{} in several important ways.
%
\begin{enumerate}
  ...
  \item We include embodied energy effects in our theory.
        We estimate embodied energy effects on rebound and compare to other effects to ascertain
...
$k$ is a dimensionless macroeconomic multiplier that accounts for 
...
thereby represents an extension of the work of \Bt{}.
...
$Re_{\incres} = k I_{\Ef} p_{\Ef}$, as shown by \Bt{}.%
\footnote{
  \Bt{} assumed that the multiplier $k = 1$. 
}
...
is as follows.
The price of gasoline ($p_{E,f}$) can be calculated using \Ba{}'s numbers as
...
\footnote{
  Note that $Re_{i} = Re_{\incres}$ when $k = 1$ in Equation~\ref{eq:Re_ir}.
}
...
Numerical quantification for each term of Equation~\ref{eq:Re_ir}
