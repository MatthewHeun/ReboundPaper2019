...
a way to understand economy-wide rebound (via a macroeconomic multiplier)
...
who used input-output methods and cross-price elasticities to
ascertain the energy cost of both substitution and income effects.
Our service price elasticity approach to substitution effects 
is identical to \Bt{}.
...
\end{equation}
%
Embodied energy rebound is a type of indirect rebound.
...
\end{equation}
%
The term $\pI$ represents 
an energy shift away from purchases in the rest of the economy
toward energy to operate the device,
a type of indirect rebound.
...
$Re_{ir}$ can be decomposed into income ($i$) and respending ($r$) terms:
$Re_{ir} = Re_i + Re_r$, with 
$Re_i \equiv (1 - \gamma) \pI$
and
$Re_r \equiv (1 - \gamma) (k - 1) \pI$.
Income effect rebound ($Re_i$) is a type of indirect rebound.
Respending rebound ($Re_r$) is a type of macroeconomic rebound.
...
%++++++++++++++++++++++++++++++
\subsection{Embodied energy effects ($Re_{emb}$)}
...
%++++++++++++++++++++++++++++++
\subsection{Device cost effects ($\gamma$)}
...
Section~\ref{sec:income_effect_derivation} in the Appendix
shows that
...
%++++++++++++++++++++++++++++++
\subsection{Device-level rebound effects ($Re_{dev}$)}
...
enhances (reduces) total rebound when the product $\pI$ is less than (greater than) 1.
Typically, $\pI < 1$, as shown in Section~\ref{sec:examples}, 
so device-level rebound typically enhances total rebound.


%++++++++++++++++++++++++++++++
\subsection{Elasticity effects ($\epsilon$)}
...
confounded by the relationship between the product $\pI$ and 1.
...
\footnote{
  ...
  \begin{enumerate*}[label={(\alph*)}]
    ...
    \item $0 < \pI < 1$ (see Section~\ref{sec:examples} below).
...
%++++++++++++++++++++++++++++++
\subsection{Multiplier, energy intensity, and energy price effects ($k$, $p_{\Ef}$, and $I_{\Ef}$)}
...
And clear thinking about the product $\pI$
provides a constraint on its value.
...
Thus, we would expect the value created by an economy 
...
while final energy price ($p_{\Ef}$) varies by energy carrier.
...
the quantity of service demanded can increase ($\rateafter{q}_s > \ratebefore{q}_s$).
The service price elasticity of final energy demand ($\epsilon$) quantifies these dynamics such that
...
However, increased energy service consumption ($\rateafter{q}_s > \ratebefore{q}_s$)
implies an increased energy cost for the device.
The increased energy cost is shifted from other expenditures elsewhere in the economy
...
such that $Re_{ir} = k Re_i$.
Rebound from respending only ($Re_r$) is given by $Re_r = (1 - \gamma) (k - 1) \pI$.
Thus, income and respending rebound is given by 
$Re_{ir} = Re_i + Re_r$.
