...
we derive a symbolic version of the \citet{Borenstein:2015aa} model
by analyzing four effects that give rise to energy rebound: 
...
%
where
a symbol (e.g., $\before{X}$ without decoration) indicates $X$ before the energy efficiency upgrade ($U$),
...
thereby represents an enhancement of the work of \citet{Borenstein:2015aa}.
...
The effect of the energy service price elasticity of final energy demand ($\epsilon$) is similarly
...
\begin{enumerate*}[label={(\alph*)}]
  ...
  \item $\epsilon < 0$ such that 
        an increase in final-energy-to-services efficiency ($\after{\eta} > \before{\eta}$)
        leads to a reduction in service price ($\after{p}_s < \before{p}_s$)
        and additional device usage.
    
\end{enumerate*}
%
When $I_{E_f} p_{E_f} < 1$, 
higher service price elasticity of demand (a more-negative $\epsilon$)
will lead to larger energy rebound, 
because $Re_{dev}$ gets more positive as demand grows and $(1 - p_{E_f} I_{E_f})$ is positive. 
On the other hand, when $I_{E_f} p_{E_f} > 1$, 
higher service price elasticity of demand (a more-negative $\epsilon$)
will read to reduced energy rebound, 
because $Re_{dev}$ gets more positive as demand grows and $(1 - p_{E_f} I_{E_f})$ is negative. 
