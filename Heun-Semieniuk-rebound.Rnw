...
% Functions defined by the authors
...
\newcommand{\ratebefore}[1]{\rate{#1}}             % Decoration for a rate before the energy efficiency upgrade (U)
\newcommand{\rateafter}[1]{\widetilde{\rate{#1}}}  % Decoration for a rate after the energy efficiency upgrade (U)
...
We extend the \cite{Borenstein:2015aa} model in several important ways.

\begin{enumerate}
 
  \item  Use typical year approach to allow for service life differences between base and EE devices.
         (Borenstein didn’t account for varying service life in the theory/equestions,
         although he may have done in the lighting example which MKH has not yet revieed in detail.)

  \item Add substitution effects to Borenstein’s simpler equation (2a).
        (Borenstein included substitution effects in equation 1 and the examples,
        but not in the simpler, usable equation, 2a.)

  \item Flesh out discussion of the multiplier.
        It is not a multiplier as typically represented (in the tax cut sense) and as Borenstein inherits.
        Rather, a technology improvement leads to growth.
        The energy implications of the new growth can be represented as a multiplier.

  \item Include multiplier
        (with additions developed by MKH recently) with income-effect terms.
        (Borenstein discussed possible multipliers but did not include k in the equations.)

  \item Re-derive the model, therby developing an explicit expression for $Re_{tot}$.
        (Borenstein had only $\Delta E$ equation. Developing the explicit expression yields additional insight.)
...
\begin{tabular}{r l}
  ...
  \midrule
  $\epsilon$ & service price elasticity of demand [--] \\ 
  $\eta$ & final energy-to-services efficiency [vehicle-miles/MJ] \\
  \bottomrule
\end{tabular}
\label{tab:greek}
\end{table}
...
\begin{tabular}{r l}
  ...
  \midrule
  $\before{X}$ & (without decoration) prior to energy efficiency upgrade \\
  $\after{X}$ & after energy efficiency upgrade \\
...
\begin{tabular}{r l}
  ...
  $i$ & an index for different rebound effects (embodied energy, substitution, income) \\
...
This section shows the derivation of the explicit rebound model.
...
income effects.
Energy rebound for each effect~($i$) can be calculated by
$Re_i = 1 - \frac{\mathrm{actual \, energy \, savings_i}}{\mathrm{expected \, energy \, savings}}$, 
...
\begin{equation} \label{eq:rebound_1}
  Re_i = \frac{\mathrm{takeback_i}}{\mathrm{expected \, energy \, savings}} \, .
\end{equation}
...
The rate of service provided~($\rate{q}_s$) is related to 
final energy-to-service efficiency~($\eta$) and 
the rate of final energy consumption directly by the device~($\rate{E}_{f,dir}$) by
$\rate{q}_s = \eta \rate{E}_{f,dir}$, where
$\rate{q}_s$ could be in [vehicle-miles/year] and
$\rate{E}_{f,dir}$ in [MJ/year].
As final energy-to-service efficiency~($\eta$) increases,
a higher rate of service~($\rate{q}_s$) can be provided for the same
rate of final energy consumption by the device~($\rate{E}_{f,dir}$).
Also, $\rate{E}_{f,dir} = \rate{q}_S / \eta$.
...
$\ratebefore{E}_{f,dir} = \ratebefore{q}_s / \before{\eta}$.
After $U$, the rate of final energy consumption by the device is
$\rateafter{E}_{f,dir} = \ratebefore{q}_s / \after{\eta}$, 
because the rate of service consumption is assumed to be unchanged 
(i.e., $\rateafter{q}_s = \ratebefore{q}_s$).
The change in the direct energy consumption rate of the device~($\Delta \rate{E}_{f,dir}$) 
due to $U$ is given by 
$\Delta \rate{E}_{f,dir} = \frac{\ratebefore{q}_s}{\after{\eta}} - \frac{\ratebefore{q}_s}{\before{\eta}}$, 
which can be rearranged to be
%
\begin{equation}
  \Delta \rate{E}_{f,dir} = \left( \frac{\before{\eta}}{\after{\eta}} - 1 \right) \ratebefore{E}_{f,dir}
\end{equation}
%
using $\ratebefore{E}_{f,dir} = \frac{\ratebefore{q}_s}{\before{\eta}}$.
if $\after{\eta} \rightarrow \infty$, 
all final energy consumption is eliminated, and 
$\Delta \rate{E}_{f,dir} = - \ratebefore{E}_{f,dir}$, as expected.
If efficiency increases due to $U$ such that $\after{\eta} > \before{\eta}$, 
the rate of final energy consumption declines, and 
$\Delta \rate{E}_{f,dir} < 0$,
as expected.

The expected, device-level energy savings is the negative of the change in direct, final energy consumption, 
or $-\Delta \rate{E}_{f,dir} = (1 - \before{\eta} / \after{\eta}) \ratebefore{E}_{f,dir}$.
Thus Equation~\ref{eq:rebound_1} can be rewritten as
%
\begin{equation} \label{eq:rebound_2}
  Re_i = \frac{\mathrm{takeback_i}}{\left(1 - \frac{\before{\eta}}{\after{\eta}} \right) \ratebefore{E}_{f,dir}} \, .
\end{equation}
...
Prior to $U$, the embodied final energy of the device is $E_{f,emb}$.
After $U$, the embodied final energy of the device is $\after{E}_{f,emb}$.
We allocate embodied energy over the life of the device ($\before{t}$ and $\after{t}$) 
to obtain embodied energy rates.
The change in embodied final energy due to $U$ is given by 
%
\begin{equation} \label{eq:delta_embodied}
  \Delta \rate{E}_{f,emb} = \left( \frac{\after{E}_{f,emb}}{\before{E}_{f,emb}}
  \frac{\before{t}}{\after{t}} - 1 \right) \ratebefore{E}_{f,emb} \, .
\end{equation}
%
Typically, increases in energy efficiency are associated with increased device complexity
and more embodied energy ($\after{E}_{f,emb} > \before{E}_{f,emb}$). 
However, if the upgraded device has longer life than the original device
($\after{t} > \before{t}$), 
$\Delta \rate{E}_{f,emb}$ can be negative.

$\Delta \rate{E}_{f,emb}$ represents energy savings taken back due to embodied energy effects.
Thus, embodied energy rebound can be written as
%
\begin{equation} \label{eq:Re_emb}
  Re_{emb} = \frac{\left( \frac{\after{E}_{f,emb}}{\before{E}_{f,emb}}\frac{\before{t}}{\after{t}} - 1 \right)}
             {\left( 1 - \frac{\before{\eta}}{\after{\eta}} \right)}
             \frac{\ratebefore{E}_{f,emb}}{\ratebefore{E}_{f,dir}} \, .
\end{equation}
%
When the original embodied energy rate is larger than the original direct final energy consumption rate 
($\ratebefore{E}_{f,emb} > \ratebefore{E}_{f,dir}$), 
embodied energy rebound will be larger.
When the embodied energy of the device is larger after $U$ than before
($\after{E}_{f,emb} > \before{E}_{f,emb}$), 
embodied energy rebound will be larger.
When the upgraded device has longer life than the original device
($\after{t} > \before{t}$), 
embodied energy rebound will be smaller.
To the extent that $U$ increases efficiency
...
($\after{p}_{E_f} = \before{p}_{E_f}$)
and using $p_s = p_{E_f}/\eta$, $\rateafter{q}_s = \ratebefore{q}_s \left( \before{\eta}/\after{\eta} \right)^\epsilon$.

As quantity demanded increases, direct energy consumption is gained 
at the device at the efficiency of the upgraded device~($\after{\eta}$),
$\rate{E}_{f,gain} = \left( \rateafter{q}_s - \ratebefore{q} \right) / \after{\eta}$.
Using $\rateafter{E}_{f,dir} = \ratebefore{q}_s / \after{\eta}$ and
$\rateafter{E}_{f,dir} = \left( \before{\eta}/\after{\eta} \right) \ratebefore{E}_{f,dir}$, 
we find that
$\rate{E}_{f,gain} = \left[ \left( \before{\eta}/\after{\eta} \right)^\epsilon - 1  \right] 
                       \left( \before{\eta}/\after{\eta} \right) \ratebefore{E}_{f,dir}$
If $\before{\eta} = \after{\eta}$, $\rate{E}_{f,gain} = 0$, as expected.
If $\epsilon = 0$, $\rate{E}_{f,gain} = 0$, as expected.

$\rate{E}_{f,gain}$ represents energy savings taken back by the substitution effect at the device.
As such, $\rate{E}_{f,gain}$ is the source of device-level rebound. 
Applying Equation~\ref{eq:rebound_2} gives 
$Re_{dev} = \rate{E}_{f,gain} / \left[ \left( 1 - \before{\eta}/\after{\eta} \right) \ratebefore{E}_{f,dir} \right]$.
After simplification, we find
%
\begin{equation} \label{eq:Re_dev}
  Re_{dev} = \frac{\left( \frac{\before{\eta}}{\after{\eta}} \right)^{\epsilon} - 1 }
